...
var arrowCodes = {
    37: "left", 38: "up", 39: "right", 40: "down", 82: "r", 81: "q",
    65: "left", 87: "up", 68: "right", 83: "down", 32: "space", 84: "t",
};
...
let walls_toggled = true;

let gui = new dat.GUI();
gui.add(CONFIG, "gravity", 0, 10);
gui.add(CONFIG, "jumpSpeed", 0, 10);
gui.add(CONFIG, "aim_dist", 0, 10);
gui.add(CONFIG, "aim_speed", 0, 3);
gui.add(CONFIG, "horSpeed", 0, 4);
...
class Tile {
    ...
    draw() {
        ...
        if (this.value === 3) {
            ctx.fillStyle = walls_toggled ? "darkred" : "darkgreen";
        } else {
...
class Vertex {
    ...
    draw() {
        ctx.fillStyle = (this == underMouse) ? "white" : "black"
        ctx.beginPath();
...
class Frame {
    ...
    // TODO: remove these two, and look at raycast()
...
class Player {
    ...
    update(step: number) {
        ...
        if (arrows.up) this.selectedAngleRadians -= step * CONFIG.aim_speed * parity;
        if (arrows.down) this.selectedAngleRadians += step * CONFIG.aim_speed * parity;

        if (arrows.left && this.inGround) this.vx = -CONFIG.horSpeed * parity;
        if (arrows.right && this.inGround) this.vx = CONFIG.horSpeed * parity;

        if (arrows.q) {
            walls_toggled = !walls_toggled;
        }
        // this.moveX(step, level);
        // this.moveY(step, level);

        // if (!this.inGround) {
        this.vy += CONFIG.gravity * step;
        // }
        if (arrows.space && this.inGround) {
            let vel = Vector2.fromRadians(this.selectedAngleRadians).mul(CONFIG.jumpSpeed);
            this.vx = vel.x;
            this.vy = vel.y;
            this.inGround = false;
        ...
        let new_frame_x = this.frame.moveX(this.vx * step);
        if (new_frame_x) {
            this.frame = new_frame_x;
        } else {
            this.vx = 0;
        }

        let new_frame_y = this.frame.moveY(this.vy * step);
        if (new_frame_y) {
            this.frame = new_frame_y;
        } else {
            this.inGround = this.vy >= 0;
            this.vy = 0;
            this.vx = 0;
        }
    }
    ...
    draw() {
        ...
        // ctx.strokeStyle = "#FF00FF";
        // // var pp = player.frame.toScreen();
        // let path = raycast(player.frame.tile, player.frame.dir, player.frame.pos.clone(), Vector2.fromRadians(this.selectedAngleRadians), 100)
        // path.forEach(function (el) {
        //     ctx.beginPath();
        //     var a = el.tile.coor_to_screen(el.p1.x, el.p1.y, el.dir);
        //     var b = el.tile.coor_to_screen(el.p2.x, el.p2.y, el.dir);
        ...
        if (this.inGround) {
            ctx.strokeStyle = "#FF00FF";
            let cur_vel = Vector2.fromRadians(this.selectedAngleRadians).mul(CONFIG.jumpSpeed);
            let cur_frame: Frame | null | undefined = player.frame;
            let cur_point = cur_frame.toScreen();
            ctx.beginPath();
            ctx.moveTo(cur_point.x, cur_point.y);
            let ds = 0.01666;
            for (let dist = 0; dist < CONFIG.aim_dist; dist += ds) {
                cur_frame = cur_frame!.moveX(cur_vel.x * ds)?.moveY(cur_vel.y * ds);
                if (!cur_frame) {
                    // finished
                    break;
                }
                cur_point = cur_frame.toScreen();
                ctx.lineTo(cur_point.x, cur_point.y);
                cur_vel.y += CONFIG.gravity * ds;
            }
            ctx.stroke();
        }
    }
